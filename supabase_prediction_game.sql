-- ============================================================
-- CWF Auction - Prediction Game SQL Setup (No Auth Version)
...
-- Insert CWF Teams (5 teams matching the auction app)
...
-- Insert all players from the auction app
...
-- 3. USERS TABLE (No Supabase Auth - phone number is the unique identifier)
...
-- ============================================================
-- 6. ROW LEVEL SECURITY (Public access - no auth required)
-- ============================================================
...
-- Allow full public access (no Supabase Auth needed)
...
-- Update Token Allocation Function (no auth.uid() - uses p_user_id param)
...
BEGIN
  ...
  SELECT id, tokens_bet INTO v_existing_prediction_id, v_existing_tokens
  FROM predictions
  WHERE user_id = p_user_id AND player_id = p_player_id AND predicted_team_id = p_team_id AND status = 'active';

  v_token_difference := p_token_amount - COALESCE(v_existing_tokens, 0);

  IF v_token_difference > v_token_balance THEN
    RAISE EXCEPTION 'Insufficient tokens. You have % tokens available.', v_token_balance;
  END IF;
  ...
  IF v_total_tokens_on_player + p_token_amount > 5 THEN
    RAISE EXCEPTION 'Maximum 5 tokens per player. Current on other teams: %, Trying to add: %',
      v_total_tokens_on_player, p_token_amount;
  END IF;

  IF v_existing_prediction_id IS NOT NULL THEN
    IF p_token_amount = 0 THEN
      DELETE FROM predictions WHERE id = v_existing_prediction_id;
      UPDATE users SET token_balance = token_balance + v_existing_tokens,
        total_predictions = GREATEST(total_predictions - v_existing_tokens, 0) WHERE id = p_user_id;
      RETURN json_build_object('success', true, 'action', 'removed',
        'tokens_remaining', v_token_balance + v_existing_tokens);
    ELSE
      UPDATE predictions SET tokens_bet = p_token_amount, updated_at = NOW() WHERE id = v_existing_prediction_id;
...
-- Process Auction Result Function
...
BEGIN
  UPDATE auction_players SET final_team_id = p_winning_team_id, final_price = p_final_price,
    auction_status = 'sold' WHERE id = p_player_id;
  ...
  FROM predictions WHERE player_id = p_player_id AND predicted_team_id = p_winning_team_id AND status = 'active';

  IF v_winning_pool = 0 THEN
    -- No winners: refund all tokens
    ...
    END LOOP;
    RETURN json_build_object('total_pool', v_total_pool, 'winning_pool', 0, 'message', 'No winners - all tokens refunded');
  ELSE
    ...
    FOR v_prediction IN SELECT * FROM predictions WHERE player_id = p_player_id AND status = 'active' LOOP
      IF v_prediction.predicted_team_id = p_winning_team_id THEN
        ...
        UPDATE users SET
          token_balance = token_balance + v_user_payout,
          tokens_won = tokens_won + v_user_payout,
          correct_predictions = correct_predictions + v_prediction.tokens_bet
        WHERE id = v_prediction.user_id;
      ELSE
        UPDATE predictions SET status = 'lost' WHERE id = v_prediction.id;
      END IF;
    END LOOP;
    -- Update accuracy for all involved users
    UPDATE users SET
